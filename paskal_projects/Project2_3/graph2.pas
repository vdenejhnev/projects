...
type
    superstring_t = array of char;          {Cтрока произвольной длины}
    supertext_t = array of superstring_t;   {Массив суперстрок}
    link = ^elem_t;
    elem_t = record                        {Очередь для поиска в ширину}
        name: superstring_t;
        next: link;
    end;
    queue_t = link;
    city_t = record                    {Запись c информацией о ребре графа}
...
begin
    ...
    begin
        writeln('There are not any files given');
        Halt(0)
    end
